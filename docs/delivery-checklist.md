...
| --- | ---: | --- | --- | --- |
| How you worked · Forma de trabajo | 24 | PR pequeños del agente, revisión escrita real del autor antes del merge, correcciones en la misma rama, historial y ramas intactos | Todos | P0–P4 integrados mediante PR #1–#5; P5 en `feat/review-queue` desde main, PR pendiente. Verificación final de comentarios de revisión pendiente |
...
| Architecture and code · Arquitectura y código | 13 | Separación de responsabilidades, código legible, lecturas en servidor y flujo completo | P0, P4–P8 | Base técnica, migraciones P1/P2/P4/P5, capa de datos `server-only` con JWT del usuario y cola de revisión con orden y cobertura en SQL (P5, sin N+1). Revisión, feedback y evidencia pendientes |
...
| Security and tenant isolation · Seguridad y aislamiento | 10 | Autorización en servidor; pruebas directas entre marcas y entre especialistas, sin depender de ocultar botones | P4, P6–P8 | Integrado (PR #5): políticas RLS por asignación de marca, especialista solo lo propio, `reviewer_id = auth.uid()`, `anon` sin privilegios. 130 aserciones pgTAP y `npm run authz-check` (16 casos por PostgREST y ruta de la app, 404 para lo ajeno) pasan. P5 añade vistas `security_invoker` limitadas a líderes; `authz-check` sube a 19 casos y pgTAP a 139 aserciones. P6–P8 deben conservarlo |
| Interface and visual craft · Interfaz y diseño | 10 | Escala tipográfica, sistema de color explicable, estados de carga/vacío/error y recorrido comprensible | P9a, P5–P9b | Tema visual (P9a) pendiente. P5: cola con estados vacíos por marca y global, mensaje para especialistas y encabezados accesibles |
...
| Ground covered in six hours · Alcance en seis horas | 5 | Producto coherente de extremo a extremo dentro del límite, con estado y horas reales | Todos | P0–P5; tiempo efectivo acumulado sin confirmar |
| **Total** | **100** | | | |
...
- [ ] Respuestas creíbles, voces distintas por marca y al menos un caso claramente malo. P3 incluye tres voces y la devolución sin diagnóstico (puntaje 1); visibles en la cola de P5 (`/review`); falta la lectura completa en el espacio de revisión (P6).
...
- [x] Prompts y registros de P0–P5 incluidos; continuar conservando los siguientes.
...
Actualizado al iniciar P5: `git fetch origin` confirma `main` en el merge de P4 mediante GitHub PR #5 (`5e93c3b`). `feat/review-queue` se creó desde ese merge y contiene P5 y los pendientes de P3/P4, implementados y verificados localmente.

Siguiente acción: revisar el PR de P5 hacia `main`, con [descripción preparada](P5-pr.md), antes de integrar mediante merge commit. Usar exclusivamente Git por SSH en la terminal y la web de GitHub para revisar e integrar el PR. El procedimiento está en [el acuerdo de ejecución](implementation-plan.md#main-as-the-integration-and-delivery-branch). La comprobación final del contenido de las revisiones humanas sigue pendiente; un merge por sí solo no prueba esa revisión. P6 no se inicia en esta entrega.
