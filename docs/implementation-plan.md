...
- The brief allows Codex and other agents; Claude Code is not a requirement. P0–P2 used Codex; P3–P10 used Claude Code.
...
P0 is merged into `main` through GitHub PR #1 (`1beec2a`), P1 including normalization through PR #2 (`2b7aba8`), P2 through PR #3 (`9295e76`), P3 through PR #4 (`f2766c1`), P4 through PR #5 (`5e93c3b`), P5 through PR #6 (`26f13f6`), P6 through PR #7 (`1dea779`), P7 through PR #8 (`64134e3`), P8 through PR #9 (`809c6c4`), and the remote HEAD points to `main`. P9b starts from that verified merge on `chore/states-polish`; P10 is stacked on it as `docs/decisions`, at the author's request. The user requires Git over SSH exclusively in the terminal. Do not use `gh` or change the working remote to HTTPS. For P2:
...
The agent prepares a [PR description](P2-pr.md) and the [comparison link](https://github.com/JuanBau514/prueba-Sellervate/compare/main...feat/quality-criteria); the author creates the PR on GitHub with base `main`, writes their review there and merges with a merge commit after corrections. This is an explicit workflow adaptation to the user's Git-only requirement; it does not replace human review with an agent's self-check. Keep the branch. The author committed P1's normalization correction as `ae05513`; it is included in the verified P1 merge. P9b is implemented on `chore/states-polish` ([PR description](P9b-pr.md), [comparison link](https://github.com/JuanBau514/prueba-Sellervate/compare/main...chore/states-polish)). Merge it before the P10 PR so that PR shows only documentation. P10 is implemented on `docs/decisions` ([PR description](P10-pr.md), [comparison link](https://github.com/JuanBau514/prueba-Sellervate/compare/main...docs/decisions)): DECISIONS.md, the final bilingual README, a measured clean-clone run and the delivery checklist.
...
| 6 | P4: server authorization and user switching | `feat/authz` | Direct API/database checks, including same-brand privacy. No `@supabase/ssr`: `fetch` + httpOnly cookies + `src/proxy.ts` refresh (follows the no-new-dependencies decision) |
| 7 | P5: review queue and coverage | `feat/review-queue` | Per-brand prioritization and distinct lead queues. Also closes P3/P4 follow-ups: home page, expired-token refresh, queue checks in `authz-check` |
| 8 | P6: contextual review workspace | `feat/review-workspace` | Atomic save, valid tags and next reply. Also brings forward most of P9a (theme, type, fonts) and fixes the queue preview, at the user's request |
